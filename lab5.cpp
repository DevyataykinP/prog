...
public:
    ...
    {
        Line::output();
        cout << "Сторона квадрата = " << sidelen << endl;
    }
    double getArea()
    {
...
public:
    ...
    Rectangle(int x1, int y1, int x2, int y2) : Square(x1, y1, x2, y2)
    {
        ...
        if (width == height)
        {
            cout << "ТЫ ЧЕ СЛЕПОЙ, НЕ ПРЯМОУГОЛЬНИК" << endl;
            exit(EXIT_FAILURE);
        }
        inp++;
        outp++;
...
int main()
{
    ...
    cout << "Создано прямоугльников: " << Rectangle::getInp() << endl;
    cout << "Существует: " << Rectangle::getOutp() << endl;
    ...
    for (int i = 0; i < 4; i++)
    {
        cout << "Периметр "<< Shapes[i]->getPer() << endl;
    }
